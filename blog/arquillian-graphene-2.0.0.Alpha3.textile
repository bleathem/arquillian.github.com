...
Graphene gets an own @@FindBy@ annotation and a @ByJQuery@ selector in order to fully support _jQuery_ selectors. ("read more":#jquery-selectors) =:
...
Waiting conditions can now be defined in nice fluent API. ("read more":#waiting-fluent-api)  =:
...
Any Arquillian resource available through @@ArquillianResource@ can be injected to page fragments. =:
...
Page Fragments received a lot of bug fixes and improvements. =:
...
The Alpha2 API allowed you to define waitings like this:

bc(prettify).. waitAjax().until(element(button).isVisible());

p. In order to fully support code-completion, we have decided to deprecate this API in favor of fluent API:

bc(prettify).. waitAjax().until().element(button).is().visible();

h3. Roadmap

This release is a significant milestone "on a way to Beta1":https://community.jboss.org/message/777443#777443.
